...
var readDatabaseConfig = function () {
    ...
    if (dir.includes('ormconfig.json')) {
        try {
            var strConfig = fs_1.readFileSync(path_1.resolve(path, 'ormconfig.json'), 'utf-8');
            var jsonConfig = JSON.parse(strConfig);
            config = database_1.transformOptions(jsonConfig);
        }
        catch (error) {
            throw error;
        }
        ...
        try {
            var file = fs_1.readFileSync(path_1.resolve(path, 'ormconfig.yml'), 'utf8');
        ...
        try {
            ...
            config = database_1.transformOptions(envConfig);
        }
        catch (error) {
            throw error;
        }
